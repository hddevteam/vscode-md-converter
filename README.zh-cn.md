...
### 🆕 v0.5.4 亮点
- **✅ Word 表格保真** - DOCX 表格现在会转换为 Markdown 表格语法，不再被压平成普通文本。
- **✅ Markdown 表格解析** - Markdown 转 Word 会保留空单元格和转义竖线。
- **🛠️ 转换诊断** - Markdown 转 Word 失败时会输出结构化诊断信息，同时不记录文档正文内容。

### 🆕 v0.5.3 亮点
- **✅ Windows 兼容性优化** - 提升了在 Windows 平台上的运行稳定性与命令执行可靠性。
- **🎯 用户价值** - 更少中断、更顺滑的转换流程，以及更好的跨平台协作一致性。
...
- **📝 增强的 Markdown 转 Word** - DOCX 生成器重大改进：
  ...
  - **嵌套列表支持** - 有序列表下的子级项目符号不再中断父级编号
  - **引用块隔离** - 引用块（Blockquote）内的列表编号与正文相互独立
  - **保留换行符** - 段落中的硬换行符现在能正确保留在 Word 文档中
  - **改进引用块渲染** - 多段落引用块不再被压缩成一行，格式更美观
- **📝 Markdown转Word** - 将Markdown文件转换为专业Word文档，支持完整格式保留
- **📋 更好的列表显示（v0.4.1）** - 使用“*”的项目符号将以符号列表呈现，不再误变成数字；子级自动缩进，结构清晰
...
### 更好的表格保真
- Word 转 Markdown 会将 DOCX 表格保留为 Markdown 表格语法
- Markdown 转 Word 会保留空单元格和转义竖线
- Markdown 转 Word 失败时会在输出通道中提供结构化诊断信息

##  规划功能

我们会继续扩展格式支持并改进转换体验：

- **🏗️ 架构改进**：为未来格式扩展提供更好的插件化基础
- **🎨 自定义模板**：支持面向不同场景的用户自定义输出模板
- **🌐 更多语言包**：在中英文之外扩展更多界面语言
- **📊 转换历史**：更方便地查看近期转换结果和输出位置
