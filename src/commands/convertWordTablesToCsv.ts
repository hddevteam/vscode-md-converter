...
import { WordTableToCsvConverter } from '../converters/wordTableToCsv';
import { TableConversionResult } from '../types';
import { I18n } from '../i18n';

export async function convertWordTablesToCsv(uri?: vscode.Uri): Promise<void> {
  try {
    ...
    if (uri && uri.fsPath) {
      ...
      const selectedFiles = await vscode.window.showOpenDialog({
        ...
        title: I18n.t('commands.convertWordTablesToCsv')
      });
    ...
    await vscode.window.withProgress({
      ...
      try {
        // Execute conversion
        const result: TableConversionResult = await WordTableToCsvConverter.convert(inputPath);
